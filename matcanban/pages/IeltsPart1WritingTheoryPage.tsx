...
import { ieltsPart1WritingTheoryData, IeltsPart1WritingTheoryQuestion } from '../data/ieltsPart1WritingTheoryData';
...
const IeltsPart1WritingTheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const quizQuestions = useMemo(() => shuffleArray(ieltsPart1WritingTheoryData).slice(0, 40), []);
    const totalQuestions = quizQuestions.length;
    const storageKey = `ielts_p1_writing_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<IeltsPart1WritingTheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<IeltsPart1WritingTheoryQuestion | null>(null);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chiến thuật Writing Task 1.</p>
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">IELTS Writing Task 1: Theory Quiz</h1>
...
export default IeltsPart1WritingTheoryPage;