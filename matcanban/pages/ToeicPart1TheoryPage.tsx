...
import { toeicPart1TheoryData } from '../data/toeicPart1TheoryData';
...
const ToeicPart1TheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const totalQuestions = useMemo(() => toeicPart1TheoryData.length, []);
    const storageKey = `toeic_p1_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<ToeicPart1TheoryQuestion[]>([]);
    const [skippedQueue, setSkippedQueue] = useState<ToeicPart1TheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<ToeicPart1TheoryQuestion | null>(null);
    ...
    const startQuiz = useCallback(() => {
        const shuffled = shuffleArray(toeicPart1TheoryData);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chiến thuật TOEIC Listening Part 1.</p>
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">TOEIC Listening Part 1: Theory Quiz</h1>
...
export default ToeicPart1TheoryPage;