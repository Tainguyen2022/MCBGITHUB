...
import { ieltsListeningTheoryData, IeltsListeningTheoryQuestion } from '../data/ieltsListeningTheoryData';
...
const IeltsListeningTheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const quizQuestions = useMemo(() => shuffleArray(ieltsListeningTheoryData).slice(0, 40), []);
    const totalQuestions = quizQuestions.length;
    const storageKey = `ielts_listening_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<IeltsListeningTheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<IeltsListeningTheoryQuestion | null>(null);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chiến thuật Listening.</p>
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">IELTS Listening: Theory Quiz</h1>
...
export default IeltsListeningTheoryPage;