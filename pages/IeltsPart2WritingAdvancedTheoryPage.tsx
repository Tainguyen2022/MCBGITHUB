...
import { ieltsPart2WritingAdvancedTheoryData } from '../data/ieltsPart2WritingAdvancedTheoryData';
import { IeltsPart2WritingAdvancedTheoryQuestion } from '../types';
import { ArrowUturnLeftIcon, TrophyIcon, CheckCircleIcon, XCircleIcon, LightBulbIcon, ClockIcon, AcademicCapIcon } from '../components/Icons';
...
const IeltsPart2WritingAdvancedTheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const quizQuestions = useMemo(() => shuffleArray(ieltsPart2WritingAdvancedTheoryData), []);
    const totalQuestions = quizQuestions.length;
    const storageKey = `ielts_p2_writing_advanced_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<IeltsPart2WritingAdvancedTheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<IeltsPart2WritingAdvancedTheoryQuestion | null>(null);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chuyên sâu Writing Task 2.</p>
                ...
                <div className="flex gap-4 mt-10">
                    ...
                    <button onClick={() => navigate('/ielts-hub')} className="btn btn-secondary w-full text-lg py-3">
            ...
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center flex items-center justify-center gap-3">
                        ...
                        IELTS Writing Task 2: Advanced Quiz
                    </h1>
...
export default IeltsPart2WritingAdvancedTheoryPage;