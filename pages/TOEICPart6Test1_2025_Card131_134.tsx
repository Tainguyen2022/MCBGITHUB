...
// Extract passage1 (questions 131-134) from toeicPart6Test1_2025
const card131_134_Passage = toeicPart6Test1_2025.passages.find(p => p.id === 'toeic-part6-test1-2025-passage1');

const TOEICPart6Test1_2025_Card131_134: React.FC = () => {
  ...
  const handleComplete = useCallback(async () => {
    ...
    const lessonId = 'part6-test1-2025-card131-134';
    ...
    >
      ...
      {showBananaReward && bananaReward && (
        <SuperGrandBananaCelebration
          ...
          course="toeic"
          lessonId="part6-test1-2025-card131-134"
      ...
      <div className="sticky top-[96px] z-50 backdrop-blur-xl bg-white/90 border-b border-gray-200/50 shadow-sm mb-4 -mx-8 px-6 py-3">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => {
                localStorage.setItem('toeic-part6-test1-2025-card131-134-answers', JSON.stringify(selectedAnswers));
                navigate('/toeic-part6-test1-2025-card131-134-explanation', {
        ...
        <div className="resizable-container flex gap-0">
            ...
            {selectedPassage.content_vi && (
              <div className="mt-4">
                <details>
                  ...
                  <div className="mt-2 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-gray-50 p-3 rounded" style={{ fontSize: 'inherit' }}>
                    {selectedPassage.content_vi}
                  </div>
                </details>
              </div>
          ...
          >
            ...
            {/* Answer Location Toggle - Apple 2025 Style - Enhanced Visibility */}
            <div className={`mb-4 flex items-center justify-between p-4 rounded-2xl border-2 shadow-md backdrop-blur-sm transition-all duration-300 ${
              highlightAnswer 
                ? 'bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-300 shadow-blue-200/50' 
                : 'bg-gradient-to-r from-gray-50 to-slate-50 border-gray-300 shadow-gray-200/50'
            }`}>
              <div className="flex items-center gap-3 flex-1">
                ...
                <div className="flex-1">
                  <span className={`text-base font-bold block tracking-tight mb-1 ${
                    highlightAnswer ? 'text-blue-900' : 'text-gray-900'
                  }`}>
                    📍 Định vị đáp án
                  </span>
                  <span className={`text-xs font-medium ${
                    highlightAnswer ? 'text-blue-700' : 'text-gray-600'
                  }`}>
                    {highlightAnswer 
                      ? '✓ Đang highlight vị trí đáp án trong passage' 
                      : 'Bật để highlight và tự động cuộn đến vị trí đáp án'}
                  </span>
                </div>
              </div>
              <button
                ...
                className={`relative inline-flex h-8 w-14 items-center rounded-full transition-all duration-300 shadow-lg ${
                  highlightAnswer 
                    ? 'bg-gradient-to-r from-blue-500 to-indigo-600 shadow-blue-400/50' 
                    : 'bg-gray-300 shadow-gray-300/50'
                } hover:scale-105 active:scale-95`}
                aria-label={highlightAnswer ? 'Tắt định vị đáp án' : 'Bật định vị đáp án'}
              >
                <span
                  className={`inline-block h-6 w-6 transform rounded-full bg-white transition-all duration-300 shadow-md ${
                    highlightAnswer ? 'translate-x-7' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
...
export default TOEICPart6Test1_2025_Card131_134;
