...
import { ieltsPart1TheoryData, IeltsPart1TheoryQuestion } from '../data/ieltsPart1TheoryData';
...
const IeltsPart1TheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const totalQuestions = useMemo(() => ieltsPart1TheoryData.length, []);
    const storageKey = `ielts_p1_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<IeltsPart1TheoryQuestion[]>([]);
    const [skippedQueue, setSkippedQueue] = useState<IeltsPart1TheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<IeltsPart1TheoryQuestion | null>(null);
    ...
    const startQuiz = useCallback(() => {
        const shuffled = shuffleArray(ieltsPart1TheoryData);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chiến thuật Speaking Part 1.</p>
                ...
                <div className="flex gap-4 mt-10">
                    ...
                    <button onClick={() => navigate('/exam-hub', { state: { selectedExam: 'IELTS' } })} className="btn btn-secondary w-full text-lg py-3">
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">IELTS Speaking Part 1: Theory Quiz</h1>
                ...
                <div className="card-base bg-white border p-6 md:p-8">
                    ...
                    {showFeedback && (
                        <div className="mt-6 space-y-4 animate-fade-in">
                            {userAnswer === '' ? (
                                <p className="text-center font-bold text-amber-600 text-xl">Hết giờ!</p>
                            ) : isCorrect ? (
                                <p className="text-center font-bold text-green-600 text-xl">Tuyệt vời! Chính xác!</p>
                            ) : (
                                <p className="text-center font-bold text-red-600 text-xl">Cố lên nào, ghi nhớ chiến thuật này nhé!</p>
                            )}
                    ...
                    <div className="mt-8 flex justify-between items-center">
                        <button 
                            onClick={() => setIsTimerlessMode(!isTimerlessMode)} 
                            className={`btn btn-secondary !rounded-full flex items-center gap-2 ${isTimerlessMode ? '!bg-blue-100 !text-blue-700' : ''}`}
                        >
                        ...
                        <div className="flex gap-4">
                            <button 
                                onClick={handleSkip} 
                                className="btn btn-secondary text-lg px-8 py-3 disabled:opacity-50 disabled:cursor-not-allowed"
                                disabled={showFeedback || (questionQueue.length === 0 && skippedQueue.length === 0)}
                            >
                                Bỏ qua
                            </button>
                            <button 
                                onClick={handleNext}
                                disabled={!showFeedback}
                                className="btn btn-writing text-lg px-8 py-3 disabled:bg-gray-300"
                            >
                                Câu tiếp theo
                            </button>
                        </div>
...
export default IeltsPart1TheoryPage;