...
import { ieltsPart2TheoryData } from '../data/ieltsPart2TheoryData';
...
const IeltsPart2TheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const totalQuestions = useMemo(() => ieltsPart2TheoryData.length, []);
    const storageKey = `ielts_p2_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<IeltsPart2TheoryQuestion[]>([]);
    const [skippedQueue, setSkippedQueue] = useState<IeltsPart2TheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<IeltsPart2TheoryQuestion | null>(null);
    ...
    const startQuiz = useCallback(() => {
        const shuffled = shuffleArray(ieltsPart2TheoryData);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài kiểm tra chiến thuật Speaking Part 2.</p>
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">IELTS Speaking Part 2: Theory Quiz</h1>
...
export default IeltsPart2TheoryPage;