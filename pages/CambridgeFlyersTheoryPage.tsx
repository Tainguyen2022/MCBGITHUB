...
import { cambridgeFlyersTheoryData } from '../data/cambridgeFlyersTheoryData';
...
const CambridgeFlyersTheoryPage: React.FC = () => {
    const navigate = useNavigate();
    const { currentUser, updateUser, addGuestBanana } = useAuth();
    const totalQuestions = useMemo(() => cambridgeFlyersTheoryData.length, []);
    const storageKey = `cambridge_flyers_theory_attempt_${currentUser?.id || 'guest'}`;

    const [questionQueue, setQuestionQueue] = useState<CambridgeFlyersTheoryQuestion[]>([]);
    const [skippedQueue, setSkippedQueue] = useState<CambridgeFlyersTheoryQuestion[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState<CambridgeFlyersTheoryQuestion | null>(null);
    ...
    const startQuiz = useCallback(() => {
        const shuffled = shuffleArray(cambridgeFlyersTheoryData);
    ...
    if (isFinished) {
        return (
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto text-center animate-fade-in mt-10">
                ...
                <p className="text-xl text-gray-600 mt-2">Bạn đã hoàn thành bài quiz lý thuyết Flyers.</p>
                ...
                <div className="flex gap-4 mt-10">
                    ...
                    <button onClick={() => navigate('/cambridge-hub/flyers')} className="btn btn-secondary w-full text-lg py-3">
        ...
        <div className="bg-slate-50 -m-8 px-4 pt-20 pb-12 min-h-screen">
            <div className="max-w-3xl mx-auto">
                <header className="mb-6">
                    <h1 className="text-4xl font-extrabold text-gray-800 text-center">Cambridge Flyers: Theory Quiz</h1>
...
export default CambridgeFlyersTheoryPage;